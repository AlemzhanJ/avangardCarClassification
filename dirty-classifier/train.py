...
class TrainConfig:
    ...
    warmup_epochs: int = 2
    model: str = "resnet50"  # "resnet50" or "efficientnet_b0"
...
def make_dataloaders(cfg: TrainConfig):
    ...
    # WeightedRandomSampler per-sample weights
...
def plot_curves(history: list, out_path: Path):
    ...
    # F1
    plt.subplot(1, 3, 3)
    plt.plot(epochs, val_macro_f1, label='val macro-F1')
    plt.xlabel('epoch')
    plt.ylabel('macro-F1')
    plt.ylim(0, 1)
    plt.grid(True, alpha=0.3)
...
def save_checkpoint(state: dict, is_best: bool, out_dir: Path, tag: str, keep_last_k: int = 3):
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = out_dir / f"checkpoint_{tag}.pt"
    torch.save(state, ckpt_path)

    # Keep only last K checkpoints (by mtime)
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device, return_probs: bool = False) -> Dict:
    ...
    # ROC-AUC (one-vs-rest); handle binary case
    roc_auc = None
    try:
        if probs.shape[1] == 2:
...
def train(cfg: TrainConfig):
    ...
    def set_backbone_trainable(trainable: bool):
        if isinstance(model, torchvision.models.ResNet):
            ...
            # efficientnet: classifier[-1] is head
            head_params = set(id(p) for p in model.classifier[-1].parameters()) if hasattr(model, 'classifier') else set()
            for p in model.parameters():
                p.requires_grad = (id(p) in head_params) or trainable
    ...
    # Optimizer and schedulers (warmup + cosine)
    ...
    # AMP setup (CUDA uses GradScaler; on MPS we use autocast only)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for images, targets in pbar:
            ...
            ctx = torch.autocast(device_type=device.type, dtype=amp_dtype) if use_amp else nullcontext()
            with ctx:
        ...
        if epoch == cfg.freeze_epochs:
            ...
            # Recreate optimizer with all params trainable
        ...
        save_checkpoint({
            ...
            'config': asdict(cfg),
            'meta': meta,
        }, is_best=False, out_dir=out_dir, tag=f"epoch{epoch:03d}", keep_last_k=cfg.save_last_k)
        ...
        if improved:
            ...
            save_checkpoint({
                ...
                'config': asdict(cfg),
                'meta': meta,
            }, is_best=True, out_dir=out_dir, tag=f"epoch{epoch:03d}", keep_last_k=cfg.save_last_k)
        else:
            epochs_no_improve += 1
        ...
        print(f"Val macro-F1: {macro_f1:.4f} | Best: {best_metric:.4f} (epoch {best_epoch})")

        # Plot curves and validation confusion matrix/ROC after each epoch
        try:
            plot_curves(history, out_dir / 'curves.png')
            class_names = [meta['idx_to_class'][0], meta['idx_to_class'][1]] if 'idx_to_class' in meta else ['class0', 'class1']
    ...
    # Test evaluation with best checkpoint if available
...
def parse_args() -> TrainConfig:
    p = argparse.ArgumentParser(description="Train binary clean/dirty classifier")
    p.add_argument('--data-dir', type=str, default='dirty-classifier/binary_dataset')
    p.add_argument('--output-dir', type=str, default='dirty-classifier/outputs/resnet50_384')
